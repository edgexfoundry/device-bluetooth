...
import org.edgexfoundry.ble.BleDriver;
import org.edgexfoundry.ble.DeviceDiscovery;
import org.edgexfoundry.ble.ObjectTransform;
...
public class BleHandler {
  ...
  private static final EdgeXLogger logger =
      EdgeXLoggerFactory.getEdgeXLogger(BleHandler.class);
  ...
  @Value("${ble.device.init:#{null}}")
  private String bleInit;
  @Value("${ble.device.init.args:#{null}}")
  private String bleInitArgs;
  @Value("${ble.device.remove:#{null}}")
  private String bleRemove;
  @Value("${ble.device.remove.args:#{null}}")
  private String bleRemoveArgs;
  ...
  public void initializeDevice(Device device) {
    if (bleInit != null && commandExists(device, bleInit)) {
      executeCommand(device, bleInit, bleInitArgs);
    }
  ...
  public void disconnectDevice(Device device) {
    if (bleRemove != null && commandExists(device, bleRemove)) {
      executeCommand(device, bleRemove, bleRemoveArgs);
    }
  ...
  private BleObject getBleObject(Map<String,
      BleObject> objects, String objectName, String transactionId) {
    BleObject object = objects.get(objectName);
  ...
  private String parseArguments(String arguments, ResourceOperation operation, Device device,
      BleObject object, Map<String, BleObject> objects) {
  ...
  private String parseArg(String arguments, ResourceOperation operation, PropertyValue value,
    ...
    } else if (value.mask().equals(BigInteger.ZERO) && !passed) {
      val = null;
    }
